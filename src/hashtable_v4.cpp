...
extern "C" unsigned int egcmp(const char*, const char*);
static inline size_t asm_memcpy_strlen_avx (char* dest, const char* src);
//=====================================================//
...
static inline size_t asm_memcpy_strlen_avx(char* dest, const char* src) {
    ...
    __asm__ volatile (
        ".intel_syntax noprefix\n"  // Используем Intel синтаксис

        "vxorps ymm0, ymm0, ymm0\n"  // ymm0 = 0 — шаблон для поиска '\0'

        ".loopi:\n"
        "vmovdqu ymm1, ymmword ptr [rsi]\n"   // Загружаем 32 байта из src
        "vpcmpeqb ymm2, ymm1, ymm0\n"        // Сравниваем с нулём побайтово
        "vpmovmskb eax, ymm2\n"              // Получаем маску совпадений
        "test eax, eax\n"
        "jnz .found_zero\n"                   // Если есть '\0', выходим

        "vmovdqu ymmword ptr [rdi], ymm1\n"  // Копируем 32 байта в dest
        "add rsi, 32\n"
        "add rdi, 32\n"
        "add %0, 32\n"
        "jmp .loopi\n"
        ...
        ".found_zero:\n"
        "bsf ecx, eax\n"                      // Находим первый '\0' в dword
        "add %0, rcx\n"                       // Добавляем offset к длине

        ".att_syntax prefix\n"                // Возвращаемся к AT&T синтаксису

        : "+r"(len)                           // Выход: len
        : "S"(src), "D"(dest)                 // Вход: src -> rsi, dest -> rdi
        : "rax", "rcx", "ymm0", "ymm1", "ymm2", "memory", "cc"
    );
...
int hashTblAdd(hashTbl_t* hashtbl, char* name){
    ...
    asm_memcpy_strlen_avx(smallBuff, name);
...
int bucketAdd(hashTbl_t* hashtbl, bucket_t** list, char* name){
    ...
    *list = (bucket_t*)calloc(1, sizeof(bucket_t));
    ...
    size_t len = asm_memcpy_strlen_avx((*list)->name, name);
if (len >= MAX_NAME) {
    (*list)->name[MAX_NAME - 1] = '\0';
} else {
    memset((*list)->name + len, 0, MAX_NAME - len);
}
